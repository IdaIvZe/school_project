...
public  class SyncRepository<T> where T : class, ISyncable
{
    ...
    //    {
    //        var local = await _localRepo.GetById(item.Id);_localRepo, 
    ...
    //public async Task GetLastSyncDate()
    //{
    //     var log = _localRepo.SyncLog.FirstOrDefaultAsync(x => x.Tablenamepor que intentasn hace )
    //}
